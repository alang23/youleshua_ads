...
class Kuaishua extends Zrjoboa
{
	...
	public function index()
	{
        ...
        $where['where']['n.merchant_id'] = '3';
		$where_count['channel_id'] = $userinfo['channel_id'];
		$where_count['merchant_id'] = '3';
        $where_count['isdel'] = '0';      
        $count = $this->business->get_count($where_count);
        //print_r($where);
        $data['count'] = $count;

        $pageconfig['base_url'] = base_url('/home/kuaishua/index?show_time='.$show_time.'&end_time='.$end_time.'&factor='.$factor.'&ads_channel='.$ads_channel.'&realname='.$realname.'&status='.$status.'&user_id='.$user_id);
     ...
     //   echo date('Y-m-d H:i:s','1501064609');
		...
		$this->tpl('home/kuaishua_tpl',$data);
	}
	...
	public function flw_index()
	{
        ...
        $pageconfig['base_url'] = base_url('/home/kuaishua/flw_index?show_time='.$show_time.'&end_time='.$end_time.'&factor='.$factor.'&phone='.$phone.'&realname='.$realname.'&status='.$status);
        $pageconfig['count'] = $count;
        $pageconfig['limit'] = $limit;
        $data['page'] = home_page($pageconfig);
        //print_r($where);
	...
	//	echo $this->db->last_query($list);	
     ...
     //   echo date('Y-m-d H:i:s','1501064609');
		...
		$this->tpl('home/kuaishua_flw_tpl',$data);
	}
	...
	public function after_index()
	{
        ...
        $pageconfig['base_url'] = base_url('/home/kuaishua/after_index?show_time='.$show_time.'&end_time='.$end_time.'&phone='.$phone.'&realname='.$realname);
        $pageconfig['count'] = $count;
        $pageconfig['limit'] = $limit;
        $data['page'] = home_page($pageconfig);
	...
	//	echo $this->db->last_query($list);	
     ...
     //   echo date('Y-m-d H:i:s','1501064609');
		...
		$this->tpl('home/kuaishua_after_tpl',$data);
	}
	...
	public function detail()
	{
		...
		$this->tpl('home/kuaishua_detail_tpl',$data);
	...
	public function edit()
	{
		if(!empty($_POST)){
			...
			if($this->business->update($update_config,$update)){
					$msg['title'] = '修改成功';
					$msg['msg'] = '<a href="'.base_url().'home/kuaishua/index?">返回列表</a> ';
			...
			$this->tpl('home/kuaishua_edit_tpl',$data);
		}
	}
	...
	public function add_record(){
		...
		if(!empty($_POST)){
			...
			$this->tpl('home/kuaishua_add_record_tpl',$data);
		}
	...
	{	
		...
		if(!empty($_POST)){
			...
			$data['info'] = $info;
			$this->tpl('home/kuaishua_record_edit_tpl',$data);
	...
	public function del_record()
	{
		...
		if($this->record->del($del_config)){
			...
			echo '<script type="text/javascript"> history.back();</script >';			
				...
				$msg['title'] = '删除失败';
				$msg['msg'] = '<a href="'.base_url().'home/kuaishua/add_record?">返回列表</a> ';
					$this->tpl('msg/msg_errors',$msg);		
		}
	}
