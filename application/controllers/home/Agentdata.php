...
class Agentdata extends Zrjoboa
{
	...
	public function index()
	{
        ...
        if(!empty($show_time)){
        	...
        	$where_count['where']['addtime >'] = $s_time;
        	if (!empty($end_time)) {
        		$e_time = strtotime($end_time);
        		$where['where']['n.addtime <'] = $e_time;
        		$where_count['where']['addtime <'] = $e_time;
        	}
        }
        ...
        if(!empty($group_id)){
        	$g_arr = explode('|', $group_id);
        	$where['where']['admin.group_id'] = $g_arr[0];
        	$where['where']['n.channel_id'] = $g_arr[1];
        }else{
        	$where['where']['n.channel_id <> '] = '10003';
        }
        ...
        if(!empty($group_id)){
        	//
        	$ids = array();
        	$ids_where['where']['group_id'] = $group_id;
        	$ids = $this->user_channel->getList($ids_where);
        	
        	foreach($ids as $k => $v){
        		$_ids[] = $v['id'];
        	}
        	
        	$where['where_in'] = array('key'=>'n.user_id','value'=>$_ids);
        ...
        */
        ...
        // $where_count['isdel'] = '0';      
        // $where_count['channel_id <> '] = '10003'; 
        $count = $this->business->get_agent_count_by_join($where);
        $data['count'] = $count;

        $pageconfig['base_url'] = base_url('/home/agentdata/index?show_time='.$show_time.'&end_time='.$end_time.'&factor='.$factor.'&ads_channel='.$ads_channel.'&realname='.$realname.'&status='.$status.'&group_id='.$group_id);
        $pageconfig['count'] = $count;
        $pageconfig['limit'] = $limit;
        $data['page'] = home_page($pageconfig);
        //print_r($where);
		$list = $this->business->get_agent_list_by_join($where);
		$data['list'] = $list;
		...
		$this->tpl('home/agentdata_tpl',$data);
	}
	...
	public function lakala(){
		...
		$list = $this->deal->get_list_by_join($where);
		$data['list'] = $list;
