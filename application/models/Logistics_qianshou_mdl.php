...
class Logistics_qianshou_mdl extends Zroa_Model
{
	
	var $table_name = 'ls_logistics_qianshou';
    ...
    public function get_list($where = array())
    {
            ...
            left join ls_admin as admin on(bus.user_id=admin.id)
            where 1=1 ".$where['where'].'  group by logis.phone  order by logis.id desc'.' limit '.$where['offset'].','.$where['limit'];
             //echo $sql;
